...
def write_result_cache(cache_key: str, result: dict[str, Any]) -> dict[str, Any]:
    if result.get("ai", {}).get("status") == "error":
        result["cache"] = {
            "hit": False,
            "backend": "not_written_due_to_ai_error",
            "key": cache_key,
            "version": "runtime_error_not_cached",
        }
        return result
...
def decide(
    ...
    if input_classification["input_type"] == IDEA:
        ...
        return write_result_cache(cache_key, result)
